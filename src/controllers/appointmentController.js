...
const getSlots = async (req, res) => {
  ...
  try {
    ...
    const doctor = await User.findById(doctorId).select('offDays isAvailable');
...
const getMyAppointments = async (req, res) => {
  try {
    ...
    // Patient: only future appointments by default
    if (req.user.role === 'patient' && !date) {
      const today = new Date().toISOString().split('T')[0];
